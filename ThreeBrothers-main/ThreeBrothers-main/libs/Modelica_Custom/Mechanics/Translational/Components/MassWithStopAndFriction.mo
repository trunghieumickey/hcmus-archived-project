within Modelica_Custom.Mechanics.Translational.Components;
...
Additionally, a left and right stop are handled.
  ...
  Modelica.Blocks.Interfaces.BooleanInput JamTrig annotation(
    Placement(visible = true, transformation(origin = {-30, 110}, extent = {{-20, -20}, {20, 20}}, rotation = -90), iconTransformation(origin = {-30, 96}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
equation
...
// Custom
  if not JamTrig then
    ...
    f = if locked then sa*unitForce else if free then 0 else (if startForward
         then 600*v + F_Coulomb + F_Stribeck else if startBackward then
        F_prop*v - F_Coulomb - F_Stribeck else if pre(mode) == Forward then
        600*v + F_Coulomb + F_Stribeck*Modelica.Math.exp(-fexp*abs(v)) else 200 -
        F_Coulomb - F_Stribeck*Modelica.Math.exp(-fexp*abs(v)));
  end if;
...
<li><em>June 10, 2002 by P. Beater, StateSelect.always for variables s and v (instead of fixed=true). </em></li>
    ...
    Icon(coordinateSystem(preserveAspectRatio=true, extent={{-100,-100},{100,100}}),
      graphics={
        ...
        Polygon(lineColor = {95, 127, 95}, fillColor = {95, 127, 95}, fillPattern = FillPattern.Solid, points = {{80, -100}, {50, -90}, {50, -110}, {80, -100}}),
        Line(points={{-40,-100},{50,-100}}, color={95,127,95}),
        Rectangle(lineColor = {0, 127, 0}, fillColor = {166, 221, 166}, fillPattern = FillPattern.Sphere, extent = {{-30, 30}, {30, -30}}),
        Rectangle(lineColor = {0, 127, 0}, fillColor = {0, 127, 0}, fillPattern = FillPattern.Solid, extent = {{-64, -16}, {-56, -46}}),
        Rectangle(lineColor = {0, 127, 0}, fillColor = {0, 127, 0}, fillPattern = FillPattern.Solid, extent = {{56, -16}, {64, -46}}),
        Text(textColor = {0, 0, 255}, extent = {{-150, 80}, {150, 40}}, textString = "%name"),
        ...
        Text(
          ...
          textString="m=%m"),
        Line(visible = false, points = {{-100, -100}, {-100, -40}, {3, -40}}, color = {191, 0, 0}, pattern = LinePattern.Dot),
        Rectangle(lineColor = {0, 127, 0}, fillColor = {160, 215, 160}, fillPattern = FillPattern.Solid, extent = {{-70, -46}, {70, -70}})}));
